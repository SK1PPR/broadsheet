...
fn probe(m: &mut Movie, ps: [usize; 3], c: Color) {
    ...
    for (j, p) in ps.iter().enumerate() {
        ...
        m.play(par![
            ...
            act().grow_to(&format!("arr{j}"), to).dur(0.5).ease(OutCubic),
...
fn insert(m: &mut Movie, word: &str, ps: [usize; 3]) {
    m.play(act().set_text("word", &format!("insert(\"{word}\")")).dur(0.3));
    m.play(act().pulse("word").dur(0.4));
    probe(m, ps, ACCENT);
    let mut flips: Vec<Clip> = Vec::new();
    for p in ps {
        flips.push(act().set_text(&format!("bit{p}.label"), "1").dur(0.2).into());
...
fn main() {
    ...
    // ---- cast ----------------------------------------------------------
    {
        ...
        s.text("key", v(-160.0, 280.0), "get(user_42)")
            ...
            .hidden();
        for i in 0..4 {
            s.rect(&format!("file{i}"), v(1030.0, 150.0 + 90.0 * i as f32), 260.0, 64.0)
                .outline_color(INK)
                .stroke(2.0)
                .hidden()
        ...
        s.rect("paper", v(390.0, 300.0), 480.0, 230.0)
            ...
            .stroke(2.0)
            .hidden();
        s.text("paper1", v(390.0, 240.0), "\u{201C}Space/Time Trade-offs in Hash Coding")
            .serif()
            .size(22.0)
            .hidden();
        ...
        s.text("paper3", v(390.0, 330.0), "Burton H. Bloom · CACM · 1970")
            ...
            .color(FADED)
            .hidden();
        s.text("paper4", v(390.0, 368.0), "5 pages. One equation. Everywhere.")
            .size(15.0)
            .color(ACCENT)
            .hidden();
        ...
        s.text("formula", v(640.0, 430.0), "k_optimal = (m / n) \u{00B7} ln 2")
            .mono_bold()
            .size(32.0)
            .color(ACCENT)
            .hidden();
        ...
        for (i, (id, title, sub)) in [
            ("vc", "COUNTING", "bits \u{2192} counters\ndeletes work · 4\u{00D7} memory"),
            ("vb", "BLOCKED", "k bits, one cache line\nwhat RocksDB ships"),
        ...
        // §6 uses
        for (i, (id, txt)) in [
            ("use1", "RocksDB · Cassandra · HBase \u{2014} skip SSTables on point reads"),
            ("use2", "Chrome \u{2014} pre-screen URLs against the malware list"),
            ("use3", "Akamai \u{2014} cache only what's been asked for twice"),
        ...
        s.text("usehead", v(640.0, 120.0), "\u{201C}CAN I SKIP THE EXPENSIVE THING?\u{201D}")
            .serif()
            .size(30.0)
            .hidden();
        ...
        s.text("outro1", v(640.0, 260.0), "Perfect answers are expensive.")
            ...
            .size(40.0)
            .hidden();
        s.text("outro2", v(640.0, 330.0), "Calibrated doubt is nearly free.")
            .serif()
            .size(40.0)
            .color(ACCENT)
            .hidden();
        s.text("next", v(640.0, 480.0), "Next issue: THE SKIP LIST \u{2014} definitely subscribe. Probably.")
            .size(20.0)
            .color(FADED)
            .hidden();
    }
    ...
    m.mark("hook");
    m.wait(2.0);
    caption(&mut m, "a read arrives \u{2014} the key could be in any of a dozen files on disk");
    m.play(par![
    ...
    m.play(par![
        ...
        act().move_to("key", v(250.0, 200.0)).dur(1.2).ease(InOutCubic),
    ]);
    m.wait(3.0);
    caption(&mut m, "opening every file to check: milliseconds, plural \u{2014} an eternity");
    m.play(seq![
    ...
    caption(&mut m, "so first it asks a tiny structure in RAM: \u{201C}have you seen this key?\u{201D}");
    m.play(par![
    ...
    m.mark("hook-reveal");
    caption(&mut m, "it can be wrong \u{2014} and being wrong in exactly one direction is the entire trick");
    ...
    m.section("1970");
    m.mark("history");
    caption(&mut m, "memory measured in kilobytes, priced like real estate");
    m.play(par![
    ...
    caption(&mut m, "Bloom's problem: hyphenation \u{2014} the whole dictionary can't fit in memory");
    m.play(par![
    ...
    m.mark("mechanics");
    caption(&mut m, "the whole machine: a bit array, all zeros \u{2014} and a few deterministic scramblers");
    {
        let ids = m.tagged("bit");
        m.play(stagger(
            0.02,
            ids.iter().map(|id| act().fade_in(id).dur(0.3).into()).collect(),
        ));
    }
    m.play(par![
    ...
    m.mark("insert-2");
    caption(&mut m, "insert \u{201C}dog\u{201D}: 3, 11, 20 \u{2014} bit 11 again. collisions are fine.");
    ...
    m.mark("insert-3");
    caption(&mut m, "insert \u{201C}fish\u{201D}: 8, 14, 26 \u{2014} overlapping fingerprints pile up");
    ...
    caption(&mut m, "one zero is proof of absence \u{2014} no disk touched");
    m.wait(7.0);
    retract(&mut m);
    m.play(act().fade_out("stampno").dur(0.4));

    m.mark("query-hit");
    caption(&mut m, "query \u{201C}cat\u{201D}: 5, 11, 26 \u{2014} all ones. answer: probably yes.");
    ...
    caption(&mut m, "a FALSE POSITIVE: other members' fingerprints happen to cover yours");
    m.wait(8.0);
    retract(&mut m);
    m.clear_all(0.8);
    m.wait(1.0);
    ...
    caption(&mut m, "never misses a real member; only occasionally cries wolf. no listing, no deletes.");
    m.wait(10.0);
    m.clear_all(0.8);
    m.wait(2.0);
    ...
    m.section("Tuning");
    m.mark("tuning");
    caption(&mut m, "how wrong is it? that's a dial you set. three knobs.");
    m.play(stagger(
        0.5,
        vec![
    ...
    m.mark("tuning-k");
    caption(&mut m, "more hashes = more specific fingerprints \u{2014} but too many lights up the whole array");
    m.play(par![
    ...
    caption(&mut m, "100 million crawled URLs \u{2192} 120 MB at 1% error. the URLs themselves: gigabytes.");
    m.wait(12.0);
    m.clear_all(0.8);
    m.wait(2.0);
    ...
    caption(&mut m, "counting: bits become counters \u{2014} deletes work, memory \u{00D7}4. nothing is free.");
    m.wait(13.0);

    m.mark("variant-blocked");
    m.play(act().fade_in("vb").dur(0.5));
    caption(&mut m, "blocked: all k bits inside one 64-byte cache line \u{2014} one memory fetch, not seven");
    m.wait(13.0);

    m.mark("variant-multilevel");
    m.play(act().fade_in("vs").dur(0.5));
    caption(&mut m, "scalable: filter full? freeze it, stack a fresh layer on top \u{2014} query newest-first");
    m.wait(13.0);

    m.mark("variant-cuckoo");
    m.play(act().fade_in("vk").dur(0.5));
    caption(&mut m, "cuckoo: the young challenger \u{2014} deletes, tighter space under 3% error");
    m.wait(13.0);
    m.clear_all(0.8);
    m.wait(2.0);
    ...
    caption(&mut m, "anywhere the question is \u{201C}can I skip the expensive thing?\u{201D}");
    m.wait(12.0);
    m.clear_all(0.8);
    m.wait(2.0);
